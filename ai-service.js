...
//   constructor(config = {}) {
//     this.provider = config.provider || 'ollama'; // 'ollama' or 'replicate'
...
//       return `${index + 1}. ${stay.name} | ${stay.type} | ${rating} | ${stay.city}`;
//     }).join('\n');
//   }

//   parseAIRecommendations(aiText, flights, stays) {
//     try {
...
//       return {
//         recommended_flight: this.selectBestFlightObject(flights),
//         recommended_hotel: this.selectBestStayObject(stays),
...
//   analyzeBudget(userDetails, flights, stays) {
//     const tripDuration = this.calculateTripDuration(userDetails.dates_start, userDetails.dates_end);
//     const cheapestFlight = flights.reduce((min, f) => f.price < min.price ? f : min);
//     const averageStayPrice = stays.reduce((sum, s) => sum + (s.price_per_night || 100), 0) / stays.length;
    
//     const estimatedCosts = {
//       flights: cheapestFlight.price * 2,
//       accommodation: averageStayPrice * tripDuration,
//       dailyExpenses: 50 * tripDuration,
//       total: 0
//     };
    
//     estimatedCosts.total = estimatedCosts.flights + estimatedCosts.accommodation + estimatedCosts.dailyExpenses;
    
//     return {
//       budgetAnalysis: {
//         userBudget: userDetails.budget,
//         estimatedTotal: estimatedCosts.total,
//         breakdown: estimatedCosts,
//         withinBudget: estimatedCosts.total <= userDetails.budget,
//         budgetAdvice: estimatedCosts.total > userDetails.budget 
//           ? `Your estimated costs ($${estimatedCosts.total}) exceed your budget. Consider shorter stays or budget accommodations.`
//           : `Your trip fits within budget with $${userDetails.budget - estimatedCosts.total} to spare for extras.`
//       }
//     };
//   }
...
//     const budget = this.analyzeBudget(userDetails, flights, stays);
